...
def ai_search(request):
    ...
    try:
        ...
        response = client.chat(
            model="gemma3:27b",
            # ← this one will need to be a variable taken from the database so the web "admin" can change it
        ...
        return JsonResponse(
            {"results": [f"Erreur : impossible de contacter l'IA pour le moment ({str(e)})"], "error": True},
            status=503)
...
context = {
    'order': order,
    'items': order.items.all(),  # On change 'cart_items' en 'items'
    'total': order.get_total_cost(),
}
    }
    return render(request, 'eshop/facture.html', context)
